  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      {baccalaureat.TypeDiplomeLibelle && (
        <div className="mb-6 p-4 bg-gray-100 rounded-lg">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-flat-fg">Baccalaureat</h4>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium text-flat-text-secondary">Type</label>
              <p className="text-sm text-flat-fg">{baccalaureat.TypeDiplomeLibelle}</p>
            </div>
            <div>
              <label className="text-xs font-medium text-flat-text-secondary">Serie</label>
              <p className="text-sm text-flat-fg">{baccalaureat.SerieDiplomeLibelle || 'Non renseignee'}</p>
            </div>
            {baccalaureat.MentionObtenueLibelle && (
              <div>
                <label className="text-xs font-medium text-flat-text-secondary">Mention</label>
                <p className="text-sm text-flat-fg font-medium">{baccalaureat.MentionObtenueLibelle}</p>
              </div>
            )}
            {baccalaureat.DateObtentionMois && (
              <div>
                <label className="text-xs font-medium text-flat-text-secondary">Date d'obtention</label>
                <p className="text-sm text-flat-fg">
            ...
            {baccalaureat.AcademieBacLibelle && (
              <div>
                <label className="text-xs font-medium text-flat-text-secondary">Academie</label>
                <p className="text-sm text-flat-fg">{baccalaureat.AcademieBacLibelle}</p>
              </div>
            )}
          </div>
          ...
          {baccalaureat.NotesBaccalaureat && baccalaureat.NotesBaccalaureat.length > 0 && (
            <div className="mt-4">
              <h5 className="text-xs font-medium text-flat-fg mb-2">Notes du baccalaureat</h5>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {baccalaureat.NotesBaccalaureat.map((note: any, index: number) => (
                  <div key={index} className="flex justify-between items-center p-2 bg-white rounded-lg border border-gray-200">
                    <span className="text-xs text-flat-text-secondary">{note.EpreuveLibelle}</span>
      ...
      <div className="space-y-4">
        <h4 className="text-sm font-semibold text-flat-fg flex items-center gap-2">
          <BookOpen className="w-4 h-4" />
          Historique scolaire
        </h4>

        {scolariteTriee.length === 0 ? (
          <p className="text-sm text-flat-text-secondary">Aucune donnee scolaire disponible</p>
        ) : (
          <div className="space-y-3">
            {scolariteTriee.map((annee: any, index: number) => (
              <div key={index} className="border-l-2 border-primary-500 pl-4 py-2">
                <div className="flex justify-between items-start mb-2">
                  <h5 className="text-sm font-medium text-flat-fg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                  <div>
                    <label className="text-xs text-flat-text-secondary">Niveau</label>
                    <p className="text-sm text-flat-fg">{annee.NiveauEtudeLibelle || 'Non renseigne'}</p>
                  </div>

                  <div>
                    <label className="text-xs text-flat-text-secondary">Type de formation</label>
                    <p className="text-sm text-flat-fg">{annee.TypeFormationLibelle || 'Non renseigne'}</p>
                  </div>

                  <div>
                    <label className="text-xs text-flat-text-secondary">Serie/classe</label>
                    <p className="text-sm text-flat-fg">{annee.SeriedeclasseLibelle || annee.ClassecandidatLibelle || 'Non renseigne'}</p>
                  </div>

                  {annee.NomEtablissementOrigine && (
                    <div className="lg:col-span-3">
                      <label className="text-xs text-flat-text-secondary">Etablissement</label>
                      <p className="text-sm text-flat-fg">{annee.NomEtablissementOrigine}</p>
                ...
                {annee.NotationScolariteLibelle && annee.NotationScolariteLibelle !== 'Non renseigne' && (
                  <div className="mt-2">
                    <label className="text-xs text-flat-text-secondary">Notation</label>
                    <p className="text-sm text-flat-fg">{annee.NotationScolariteLibelle}</p>
                  </div>
                )}
              </div>
            ))}
      ...
      {(candidat.moyenneGenerale || candidat.moyenneFrancais || candidat.moyennePhilosophie) && (
        <div className="mt-6 p-4 bg-gray-100 rounded-lg">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-sm font-semibold text-flat-fg">Moyennes</h4>
            <TrendingUp className="w-3.5 h-3.5 text-flat-text-secondary" />
          </div>
